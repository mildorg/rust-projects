...
#[hook]
pub fn use_ripple(center: bool, timeout: u32, class: Option<Classes>) -> UseRipple {
...
fn stop(
    ...
    Callback::from(move |_| {
        if !ripples.is_empty() {
            let pre_clear_ref = clear_ref.clone();
            let ripples = ripples.clone();

            let clear = set_timeout(timeout, move || {
                // clear the previous timer
                clear_timeout(*pre_clear_ref.borrow());
...
fn create_ripple(params: PCreateRipple) {
    let PCreateRipple {
        el,
        center,
        client_x,
...
fn get_coordinates(params: PCoordinates) -> (f64, f64) {
    let PCoordinates {
        rect,
        client_x,
        client_y,
...
fn get_ripple_size(params: PSize) -> f64 {
    let PSize {
        el,
        rect,
        ripple_x,
